...
SnapSerializer.prototype.app = "CSnap! Pro" + SnapVersion + ", http://csdt.org";
...
SnapSerializer.prototype.loadScene = function (xmlNode, appVersion, remixID) {
	...
	if (!scene.name) {
		nameID = 1;
		while (Object.prototype.hasOwnProperty.call(localStorage, "-snap-project-Untitled " + nameID)) {
			nameID += 1;
		}
	...
	scene.penColorModel = model.scene.attributes.colormodel === "hsl" ? "hsl" : "hsv";
	...
	if (model.stage.attributes.color) {
		...
		scene.stage.cachedColorDimensions = scene.stage.color[SpriteMorph.prototype.penColorModel]();
	}
	...
	BooleanSlotMorph.prototype.isTernary = model.stage.attributes.ternary !== "false";
	...
	if (model.hiddenPrimitives) {
		hidden = model.hiddenPrimitives.contents.split(" ").filter((word) => word.length > 0);
		if (hidden.length) {
			hidden.forEach((sel) => {
			...
			SpriteMorph.prototype.newPrimitivesSince(appVersion).forEach((sel) => {
				var selector, migration;
				if (sel) {
					migration = SpriteMorph.prototype.blockMigrations[sel];
					selector = migration ? migration.selector : sel;
	...
	if (model.codeHeaders) {
		model.codeHeaders.children.forEach((xml) => (scene.codeHeaders[xml.tag] = xml.contents));
	}

	model.codeMappings = model.scene.childNamed("code");
	if (model.codeMappings) {
		model.codeMappings.children.forEach((xml) => (scene.codeMappings[xml.tag] = xml.contents));
	}
	...
	model.sprites.childrenNamed("sprite").forEach((model) => this.loadValue(model));
	...
	this.scene.stage.children.forEach((sprite) => {
		...
		if (sprite.inheritsAttribute("costumes")) {
			if (sprite.inheritsAttribute("costume #")) {
				...
				costume = sprite.costumes.asArray()[sprite.inheritanceInfo.costumeNumber - 1];
			}
			if (costume) {
				if (costume.loaded) {
	...
	model.sprites.childrenNamed("watcher").forEach((model) => {
		...
		if (watcher.currentValue instanceof List && watcher.cellMorph.contentsMorph) {
	...
	this.scene.stage.children.forEach((sprite) => (sprite.inheritedMethodsCache = []));
